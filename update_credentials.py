...
def update_m3u_file(new_username, new_password):
    ...
    pattern = re.compile(r'(https?://(?:www\.)?freeiptv(?:\d*-d)?\.ottc\.xyz(?::\d+)?/live/)([^/]+)/([^/]+)(/[^ \n"\']*)',
                         flags=re.IGNORECASE)
...
def fetch_credentials_with_playwright(url, max_wait_seconds=90):
    try:
        ...
        with sync_playwright() as p:
            ...
            if not clicked:
                try:
                    ...
                    log.info("Attempted JS-click for create control.")
                except Exception:
                ...
                try:
                    ...
                    if user_present:
                        try:
                            ...
                            u_val = u_el.get_attribute("value") or u_el.input_value() or u_el.inner_text() or None
                        except Exception:
                            try:
                    ...
                    if pass_present:
                        try:
                            ...
                            p_val = p_el.get_attribute("value") or p_el.input_value() or p_el.inner_text() or None
                        except Exception:
                            try:
                    ...
                    log.debug("Loop exception while waiting for credentials: %s", e)
...
def get_credentials():
    ...
    u, p = fetch_credentials_with_playwright(TARGET_URL, max_wait_seconds=90)
    if u and p:
        return u, p
