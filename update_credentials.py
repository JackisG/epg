...
def fetch_credentials_via_oxylabs():
    """
    Use Oxylabs Web Scraper API to solve Turnstile, click the button,
    and retrieve the credentials page HTML.
    """
    payload = {
        "source": "universal",
        "url": INDEX_URL,
        "render": "html",               # Enable JavaScript
        "captcha": {
            "solve": True,              # Automatically solve Turnstile
            "provider": "oxylabs"       # default
        },
        "actions": [
            {
...
def parse_credentials(html):
    """Extract username and password from the credentials page."""
    soup = BeautifulSoup(html, "html.parser")
    user_input = soup.find("input", {"id": "accUser"})
    pass_input = soup.find("input", {"id": "accPass"})
    if not user_input or not pass_input:
        raise RuntimeError("Credentials fields not found in HTML")
